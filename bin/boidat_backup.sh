#!/usr/bin/env bash
#
# Nightly backup of the Boidat production database and uploaded files.
#
#   Runs from cron as the deploy user:
#     15 3 * * * /var/www/boidat/shared/bin/boidat_backup.sh >> /var/www/boidat/shared/log/backup.log 2>&1
#
#   Keeps BACKUP_KEEP_DAYS (default 30) of daily snapshots in /var/www/boidat/backups,
#   mode 700 — the dumps contain every tenant's contracts, bills and ID documents.
#
# RESTORE:
#   dropdb  -h localhost -U boidat boidat_production
#   createdb -h localhost -U boidat boidat_production
#   pg_restore -h localhost -U boidat -d boidat_production --no-owner db/boidat_YYYYmmdd.dump
#   tar xzf storage/storage_YYYYmmdd.tar.gz -C /var/www/boidat/shared
#
...
APP_ROOT="${APP_ROOT:-/var/www/boidat}"
...
DB_PASS="$(env_get BOIDAT_DATABASE_PASSWORD)"
[ -n "$DB_PASS" ] || fail "missing BOIDAT_DATABASE_PASSWORD in $ENV_FILE"
DB_NAME="$(env_get BOIDAT_DATABASE_NAME)"; DB_NAME="${DB_NAME:-boidat_production}"
DB_USER="$(env_get BOIDAT_DATABASE_USER)"; DB_USER="${DB_USER:-boidat}"
DB_HOST="$(env_get BOIDAT_DATABASE_HOST)"; DB_HOST="${DB_HOST:-localhost}"

mkdir -p "$BACKUP_ROOT/db" "$BACKUP_ROOT/storage"
chmod 700 "$BACKUP_ROOT"

# ---- Database ---------------------------------------------------------------
DUMP="$BACKUP_ROOT/db/boidat_${STAMP}.dump"
