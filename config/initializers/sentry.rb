...
Sentry.init do |config|
  ...
  # Belt and braces on top of Rails' own filter_parameters.
  filter = ActiveSupport::ParameterFilter.new(
    Rails.application.config.filter_parameters +
    %i[account_no account_name phone email otp code token file readings]
  )
  ...
  # Performance tracing is billed separately and we do not need it yet.
